...
class SwimStore {
  ...
  visibleGoalTimeColumns: (keyof GoalTime)[] = ['email', 'stroke', 'distance', 'gear', 'time'];
    ...
    onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        if (!querySnapshot.empty) {
          ...
          // This case should ideally not happen if users are created via register
          // Or if a user signs in with a provider (e.g., Google) for the first time
          // For now, we'll just set a basic user or handle as needed.
          runInAction(() => {
            this.currentUser = {
              id: firebaseUser.uid,
              name: firebaseUser.displayName || firebaseUser.email!.split('@')[0],
              email: firebaseUser.email!,
              isAdmin: false, // Default to false, can be updated in Firestore
            };
          });
        }
      } else {
  ...
  async register(name: string, email: string, password: string, stravaClientId?: string, stravaClientSecret?: string) {
 ...
 get velocityDistanceData() {
  ...
  get uniqueSwimmers() {
    // Only show current user's name if logged in, otherwise all swimmers from swims
    if (this.currentUser) {
