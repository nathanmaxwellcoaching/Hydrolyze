...
const groupSwimsIntoSeries = (swims: Swim[]) => {
  ...
  swims.forEach((swim) => {
    ...
    // Look up user name from swimStore.users using swim.UID
    const user = swimStore.users.find((u) => u.UID === swim.UID);
    const displayName = user ? user.name : swim.UID; // Fallback to UID if user not found
    const seriesKey =
      `${displayName}-${swim.distance}m-${swim.stroke} ${gearStr}`.trim();
  ...
  return Object.entries(groups).map(([key, g]) => {
    ...
    return {
      name: key.length > 45 ? `${key.slice(0, 42)}…` : key,
      data: g.data,
    };
  });
};
...
const prepareChartData = (
  ...
  const groups: Record<
    string,
    { name: string; data: [number, number, Swim][] }
  > = {};
...
const SwimTimesChart = observer(() => {
  ...
  const timeSeries = useMemo(
    () => groupSwimsIntoSeries(filteredSwims),
    [filteredSwims]
  );
  const dynamicSeries = useMemo(
    () => prepareChartData(filteredSwims, yMetric, xAxis),
    [filteredSwims, yMetric, xAxis]
  );
  ...
  const yMetricText =
    yMetric === "SI" ? "Swim Index (SI)" : "Efficiency Index (IE)";
  ...
  const commonOptions: ApexOptions = {
    ...
    colors: [
      ...
      "var(--color-accent-light-lavender)",
    ],
  ...
  const timeSeriesOptions: ApexOptions = {
    ...
    xaxis: {
      type: "datetime",
      labels: {
        formatter: (_, ts) => moment(ts).format("DD MMM"),
    ...
    yaxis: {
      ...
      labels: {
        formatter: (val) =>
          `${Math.floor(val / 60)
            .toString()
            .padStart(2, "0")}:${(val % 60).toFixed(0).padStart(2, "0")}`,
        style: { colors: "var(--color-text-secondary)" },
      },
    },
    tooltip: {
      ...
      y: {
        formatter: (val) =>
          `${Math.floor(val / 60)
            .toString()
            .padStart(2, "0")}:${(val % 60).toFixed(1).padStart(4, "0")}`,
      },
    },
  };
  ...
  const dynamicChartOptions: ApexOptions = {
      ...
      events: {
        dataPointSelection: (_, __, config) => {
          ...
          if (
            selectedSeries &&
            selectedSeries.data &&
            selectedSeries.data[dataPointIndex]
          ) {
    ...
    xaxis: {
      ...
      title: {
        text: xAxis === "Date" ? "Date" : "Velocity (m/s)",
        style: {
          color: "var(--color-text-secondary)",
          fontSize: "12px",
          fontWeight: 500,
    ...
    tooltip: {
      ...
      x: {
        formatter: (val, { dataPointIndex, seriesIndex, w }) => {
          const swim = w.config.series[seriesIndex].data[
            dataPointIndex
          ][2] as Swim;
          if (xAxis === "Date") return moment(val).format("DD MMM YYYY");
          return `${Number(val).toFixed(2)} m/s (${moment(swim.date).format(
            "DD MMM YYYY"
          )})`;
        },
      },
  ...
  return (
    <div>
      ...
      {dynamicSeries.length > 0 && (
        <div className="mt-4">
          ...
          <div className="flex items-center justify-center space-x-4 mt-4 p-2 bg-gray-800/50 rounded-md">
            ...
            <div className="flex items-center space-x-2">
              <label htmlFor="x-axis-select" className="text-sm text-gray-300">
                X-Axis:
              </label>
              <select
                id="x-axis-select"
                value={xAxis}
                onChange={(e) =>
                  setXAxis(e.target.value as "Date" | "Velocity")
                }
                className="form-select block w-full pl-3 pr-10 py-2 text-base border-gray-600 bg-gray-700 text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
              >
            ...
            <div className="flex items-center space-x-2">
              <label htmlFor="trend-select" className="text-sm text-gray-300">
                Show Trendlines
              </label>
              <input
                id="trend-select"
            ...
            <div className="flex items-center space-x-2">
              <label htmlFor="std-dev-select" className="text-sm text-gray-300">
                Show 2x Standard Deviation
              </label>
              <input
                id="std-dev-select"
                type="checkbox"
      ...
      {showTrendlines &&
        (timeTrendStats.length > 0 || dynamicTrendStats.length > 0) && (
          <Accordion
          ...
          >
            <AccordionSummary
              expandIcon={
                <ExpandMoreIcon sx={{ color: "var(--color-text-secondary)" }} />
              }
            >
              <b className="text-lg text-gray-300">Trendline Statistics</b>
            </AccordionSummary>
            <AccordionDetails>
              <h5 className="text-md text-gray-300 mt-2">
                Swim Duration Over Time
              </h5>
              <ul className="list-disc pl-5 text-sm text-gray-400">
                {timeTrendStats.map((stat, index) => (
                  <li key={index}>
                    {stat.name}: r² = {stat.rSquared}
                  </li>
                ))}
              </ul>
