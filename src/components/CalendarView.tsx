...
import { Box, Typography, Tooltip, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Alert } from '@mui/material';
...
const CalendarView = observer(() => {
  ...
  const dailyActivity = useMemo(() => {
    ...
    const userEmail = swimStore.currentUser?.email;
    if (!userEmail) return activities;

    swimStore.stravaSessions.forEach(session => {
      if (session.swimmerEmail === userEmail) {
    ...
    swimStore.swims.forEach(swim => {
      if (swim.swimmerEmail === userEmail) {
    ...
    return activities;
  }, [swimStore.swims, swimStore.stravaSessions, swimStore.currentUser]);
  ...
  const selectedDayActivities = useMemo(() => {
    ...
    const stravaActivities = swimStore.stravaSessions
      .filter(s => s.swimmerEmail === userEmail && getDateKey(new Date(s.start_date)) === selectedKey)
      .map(s => ({ ...s, type: 'Strava' }));

    const swimActivities = swimStore.swims
      .filter(s => s.swimmerEmail === userEmail && getDateKey(new Date(s.date)) === selectedKey)
  ...
  }, [selectedDate, swimStore.swims, swimStore.stravaSessions, swimStore.currentUser]);
