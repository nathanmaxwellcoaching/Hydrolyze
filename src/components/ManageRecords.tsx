...
const ManageRecords = observer(() => {
  ...
  const handleDrawerToggle = () => {
    // Handled by parent Layout; keep noop to satisfy Header prop
  };
  ...
  const isEditable = (key: any) => !['id', 'velocity', 'sl', 'si', 'ie'].includes(key);
  ...
  return (
    <Box ref={pageRef} sx={{ opacity: 0 }}>
      {/* NEW: Header bar with filters/columns/toggles */}
              ...
              {swimStore.filteredSwims.map((record) => (
                <TableRow key={record.id} sx={{ '&:hover': { backgroundColor: 'rgba(255,255,255,0.05)' } }}>
                  {swimStore.visibleColumns.map((column) => (
                    <TableCell
                      key={column}
                      sx={{ color: 'var(--color-text-secondary)', borderBottom: '1px solid var(--color-border)' }}
                    >
                  ...
                  <TableCell sx={{ borderBottom: '1px solid var(--color-border)' }}>
                    {editState[record.id] ? (
                      <Button
                        ...
                        variant="contained"
                        size="small"
                        sx={{ backgroundColor: 'var(--color-accent-green)', mr: 1 }}
                      >
                        Save
                      </Button>
                    ) : (
                      <Button
                        ...
                        variant="outlined"
                        size="small"
                        sx={{ borderColor: 'var(--color-accent-yellow)', color: 'var(--color-accent-yellow)', mr: 1 }}
                      >
                        Edit
                      </Button>
                    )}
