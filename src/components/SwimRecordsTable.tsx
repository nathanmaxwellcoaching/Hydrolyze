...
const columnDisplayNames: { [key in keyof Swim]?: string } = {
  ...
  si: 'SI',
  ie: 'IE Ratio',
  stravaActivity: 'Strava Activity', // Added
  race: 'Race', // Added
};
...
const getColumnValue = (record: Swim, column: keyof Swim) => {
  if (column === 'UID') {
    const user = swimStore.users.find((u) => u.UID === record.UID);
    return user ? user.name : record.UID; // Display name or fallback to UID
  }

  if (column === 'race') {
    return record.race ? '✓' : '✗'; // Display check symbol if true, cross symbol if false
  }
...
const SwimRecordsTable = observer(() => {
  ...
  useEffect(() => {
    if (tableBodyRef.current) {
      fadeInWithBlur(
        (tableBodyRef.current as HTMLElement).children,
        anime.stagger(75)
      );
    }
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <FormControl
            sx={{ minWidth: 140, ...formControlStyles }}
            size="small"
          >
            ...
            {swimStore.filteredSwims.map((record) => (
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  Time: {record.duration}s
                </Typography>
                {record.targetTime && (
                  <Typography variant="body2" color="text.secondary">
                    Target: {record.targetTime}s
                  </Typography>
                )}
              </Paper>
            ))}
