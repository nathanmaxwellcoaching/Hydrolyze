...
// Helper: average *signed* % difference for all filtered swims that have a target
const getAvgPercentDiff = () => {
  ...
  const sum = swims.reduce((acc, s) => {
    const pct = ((s.duration - s.targetTime!) / s.targetTime!) * 100; // ← signed
    return acc + pct;
  }, 0);
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={6} lg={3}>
          <SummaryCard
            ...
            value={
              (() => {
                const avg = getAvgPercentDiff();
                return avg !== null ? `Δ ${avg.toFixed(2)}%` : 'N/A';
              })()
            }
            subValue="Average % Difference"
            customContent={
              swimStore.achievementZoneDistribution &&
              swimStore.achievementZoneDistribution.length > 0 ? (
