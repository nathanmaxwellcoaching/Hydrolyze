            ...
            <TableBody>
              {Array.isArray(swimStore.users) && swimStore.users.map((user) => (
                <TableRow key={user.id} sx={{ '&:hover': { backgroundColor: 'rgba(255,255,255,0.05)' } }}>
                  ...
                  <TableCell sx={{ color: 'var(--color-text-secondary)', borderBottom: '1px solid var(--color-border)' }}>
                    {editState[user.id] ? <Checkbox checked={editState[user.id]?.is_admin || false} onChange={(e) => handleEditChange(user.id, 'is_admin', e.target.checked)} sx={{ color: 'var(--color-text-secondary)', '&.Mui-checked': { color: 'var(--color-accent-orange)' } }} /> : (user.is_admin ? 'Yes' : 'No')}
                  </TableCell>
