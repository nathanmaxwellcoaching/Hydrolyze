...
const FilterCard = observer(({ sx }: { sx?: object }) => {
  useEffect(() => {
  ...
  return (
    <Card sx={{ backgroundColor: '#1A1A1A', color: '#FFFFFF', mt: 2, ...sx }}>
      <CardContent>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 2 }}>
          {(swimStore.currentUser?.isAdmin || swimStore.currentUser?.userType === 'coach') && (
            <FormControl fullWidth>
              <InputLabel sx={{ color: '#B0B0B0' }}>Swimmer (Name)</InputLabel>
              <Select
                value={swimStore.activeFilters.swimmerUID || ''}
                sx={{ color: '#FFFFFF', '& .MuiSvgIcon-root': { color: '#B0B0B0' } }}
                onChange={(e) => swimStore.applyFilters({ swimmerUID: e.target.value as string })}
              >
                <MenuItem value=""><em>All</em></MenuItem>
                {(swimStore.currentUser?.isAdmin ? swimStore.users : swimStore.swimmerUsers).map((user) => (
